...
const data = JSON.parse(fs.readFileSync('./JsonMakeFiles/dataFilter.json', 'utf8'));

// Crop category names mapping
const cropCategories = JSON.parse(fs.readFileSync('./JsonMakeFiles/cropCategories.json', 'utf8'));

// Import the (dC mapping) errorCorrection JSON file
const dcMapping = JSON.parse(fs.readFileSync('./JsonMakeFiles/dCMappingErrorCorrection.json', 'utf8'));

const baseUrl = 'https://www.statistics.gov.lk/HIES/HIES2006_07Website/HighlandCrops.asp?getcode=Table&dL=011112132122233132334142434445515253616271728182919299&P=1&E=1';
...
async function fetchData() {
    ...
    for (const mapping of dcMapping) {
        ...
        try {
            ...
            console.log(`Data for crop category ${cropCatNumber} with corrected dC saved.`);
        } catch (error) {
            console.error(`Error fetching data for crop category ${cropCatNumber} with corrected dC:`, error);
            errorLog.push({
    ...
    if (fs.existsSync('./data/researchData.json')) {
        existingData = JSON.parse(fs.readFileSync('./data/researchData.json', 'utf8'));
    }
    ...
    try {
        ...
        fs.writeFileSync('./data/researchData.json', JSON.stringify(mergedData, null, 2));
