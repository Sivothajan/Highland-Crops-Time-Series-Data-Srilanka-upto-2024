...
const baseUrl = 'https://www.statistics.gov.lk/HIES/HIES2006_07Website/HighlandCrops.asp?getcode=Table&dC=27&dL=011112132122233132334142434445515253616271728182919299&P=1&E=1';
...
async function fetchData() {
  ...
  for (const cropCat of data.C) { // Assuming data.C is an array, so we iterate through it
    ...
    for (const [cropCatNumber, years] of Object.entries(cropCat)) {
      ...
      try {
        ...
        console.error(`Error fetching data for crop category ${cropCatNumber}:`, error);
        errorLog.push({
  ...
  try{
    ...
    fs.writeFileSync('./data/researchData.json', JSON.stringify(allTableData, null, 2));
